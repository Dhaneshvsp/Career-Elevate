...
//   const [shouldSpeak, setShouldSpeak] = useState(true); // ✅ new state
...
//       if (res && res.length > 0) {
//         const jsonMockResp = res[0].jsonMockResp;
//         const cleanedJsonResp = jsonMockResp
//           .trim()
//           .replace(/^\ufeff/, "")
//           .replace(/```/g, "");
//         const parsedMockResp = JSON.parse(cleanedJsonResp || "{}");
//         const questions = Array.isArray(parsedMockResp)
//           ? parsedMockResp
//           : [];
//         setMockInterviewQuestion(questions);
//         setInterviewData(res[0]);
...
//     const timeout = setTimeout(() => {
//       clearInterval(timer);
//       setShouldSpeak(false); // stop current speech
//       window.speechSynthesis.cancel();
//       setActiveQuestionIndex((prev) =>
//         prev < mockInterviewQuestion.length - 1 ? prev + 1 : prev
//       );
//       setTimeout(() => setShouldSpeak(true), 300); // allow speech for new question
//     }, 180000);
//     return () => {
//       clearInterval(timer);
//       clearTimeout(timeout);
...
//                 onClick={() => {
//                   window.speechSynthesis.cancel(); // 🔇 stop on exit
//                   setShouldSpeak(false);
//                 }}
//               >
//                 <Button className="bg-black text-white px-6 py-3 rounded-lg hover:bg-gray-500">
